    ...
    <div className="home_saved_list">
      ...
      {saveCourseList.length > 0 ? (
        saveCourseList.map((data) => {
          return (
            <div
              className="saved_card_wrap wow fadeInRight"
              key={data._id}
              onClick={() => navigate(`/explore/${data?.course_id?._id}`)}
            >
              <button
                type="button"
                className="saved_btn"
                onClick={(e) => saveBtnHandler(e, data?.course_id?._id)}
              >
